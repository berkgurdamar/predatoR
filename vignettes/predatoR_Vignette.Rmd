...
`predatoR` package makes prediction about amino acid mutations based on a pre-computed __Adaboost__ model (Accuracy: 0.9140, Sensitivity: 0.9667, Specificity: 0.8005, AUC: 0.9718) and classifies the mutation as __Disease Causing__ or __Silent__. Model created by using [Varibench](http://structure.bmc.lu.se/VariBench/) datasets. [Missense3D](http://missense3d.bc.ic.ac.uk/~missense3d/) datasets also used for additional model validation (Accuracy: 0.7818, Sensitivity: 0.8174, Specificity: 0.7428, AUC: 0.8601). 
...
`KEGG_pathway_number()` function finds number of KEGG Pathways which contains the input genes.
