...
`predatoR()` function works on each PDB ID respectively. For each PDB ID;  
 ...
 - Gets [gnomAD](https://gnomad.broadinstitute.org/) Synonymous Z-Score, Non-Synonymous Z-Score, and pLI Score
...
`gnomad_scores()` function gets input gene's gnomAD Synonymous Z-Score, Non-Synonymous Z-Score, and PLI Score
...
If there are multiple genes annotated for same PDB, `gnomad_scores()` function gets the gene that has maximum Synonymous Z-Score, Non-Synonymous Z-Score, and pLI Score.
