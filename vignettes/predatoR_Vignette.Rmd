...
`predatoR()` function is the wrapper function of the `predatoR` package. `predatoR()` gets a data.frame structure as an input. Input should contain 5 mandatory and 1 optional columns. Mandatory columns are  __'PDB_ID'__, __'Chain'__, __'Position'__, __'Orig_AA'__ and  __'Mut_AA'__. __'Gene_Name'__ column is optional. Predictions can be made by using 2 different models, 7 Angstrom (Å)-all atoms model and 5Å-carbon alpha (Cα) atoms only model.
...
### Arguments
 ...
 - __distance_cutoff:__ distance cutoff for setting edges (default = 7)
 - __network_approach:__ network building approach; "all" (default) for using all atoms or "ca" for using Cα atoms only
 ...
 genes annotated for the PDB ID, choose the gene having the higher gnomAD metrics. 
 
 `predatoR()` can make predictions using 7Å or 5Å distance cutoffs. For exploratory purposes, different cutoffs can be used and network properties can be calculated.
...
test_data <- as.data.frame(rbind(c("3SQJ", "A", 196, "GLN", "LEU"),
...
prediction_result <- predatoR(test_data, distance_cutoff = 7.6, network_approach = "all") 

# networks build using only Cα atoms and 8Å cutoff
prediction_result <- predatoR(test_data, distance_cutoff = 8, network_approach = "ca") 
...
and creates an edge between atoms if the distance <= 7Å. As a result, returns a list contains data frames of different chains' interaction lists.
...
`clique_score()` function calculates Clique Z-scores score of each atom and returns the desired nodes' scores.
...
If there are multiple genes annotated for same PDB Chain ID, `gnomad_scores()` function gets the gene that has maximum Synonymous Z-Score, Missense Z-Score, and pLI Score.
...
`KEGG_pathway_number()` function finds the number of KEGG Pathways associated with the input genes.
...
`GO_terms()` function returns the number of GO terms associated with the input genes.
...
`DisGeNET()` function returns number of diseases associated with input genes by using a dataset retrieved from 
...
[GTEx](https://gtexportal.org/home/) and returns the values of input genes.
...
final_df <- as.data.frame(rbind(strsplit("3SQJ, A, 196, GLN, LEU, ALB, -0.0717218071350537, -0.379784447875971, -1.61712172099028, 0.709536957153219, -0.0906809533248494, 0.0255568093251824, -0.91513, 0.65574, 0.64329, -2, 12, -0.89, 4, 81, 1, 1.515105, 177.7, 173.7, -4, -3.68, 3.8, 7.48", ", ")[[1]],
                                strsplit("3SQJ, A, 396, GLU, LYS, ALB, -0.280119006995483, -0.406648724044331, -0.114702339705536, -0.531050971889894, 0.0706892006523297, -0.318847672527394, -0.91513, 0.65574, 0.64329, 1, 12, -0.89, 4, 81, 1, 1.515105, 182.9, 215.2, 32.3, -3.2, -4.11, -0.91", ", ")[[1]]))
