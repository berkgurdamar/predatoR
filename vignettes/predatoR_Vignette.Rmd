...
`predatoR` package makes prediction about amino acid mutations based on a pre-computed __Adaboost__ model (Accuracy: 0.9043, Sensitivity: 0.9572, Specificity: 0.7903) and classifies the mutation as __Disease Causing__ or __Silent__. Model created by using [Varibench](http://structure.bmc.lu.se/VariBench/) datasets. [Missense3D](http://missense3d.bc.ic.ac.uk/~missense3d/) datasets also used for additional model validation (Accuracy: 0.7748, Sensitivity: 0.8224, Specificity: 0.7211). 
...
Calculates Z-scores of each node and returns desired nodes scores.

#### Arguments

`eigen_centrality_score()` has 2 arguments;

 - __edge_list:__ Matrix created by `PDB2connections()` function
 - __filtered_info_df:__ data.frame which contain only one PDB entries
...
function of igraph package, turns the scores into Z-scores and returns desired nodes scores.
